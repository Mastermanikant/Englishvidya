...
async function onRequestGet2(context) {
  try {
    ...
    if (!user) {
      ...
      let result;
      const insertUser = /* @__PURE__ */ __name(async (gId, uEmail, uName, uPic, ip, refId) => {
        try {
          if (refId) {
...
async function onRequestPost4(context) {
  ...
  if (Array.isArray(references)) {
    const validRefs = references.filter((l) => typeof l === "string" && l.trim().length > 0).map((l) => l.trim()).filter((l) => {
      try {
...
function getCookieValue(cookieString, name) {
  ...
  if (!match2)
    return null;
  try {
    return decodeURIComponent(match2[1]).replace(/^"|"$/g, "").trim();
  } catch (e) {
...
// ../.wrangler/tmp/pages-dfvipz/functionsRoutes-0.6630037031474354.mjs
var routes = [
  {
