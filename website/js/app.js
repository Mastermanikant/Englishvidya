...
// ═══════════════════════════════════════════════════
//  14. GLOBAL INTERACTIVE HELPERS (TTS, Quiz, Counters, Keyboard Shortcuts)
// ═══════════════════════════════════════════════════
window.speakText = function(text) {
  if ('speechSynthesis' in window) {
    ...
    utterance.onstart = function() {
      if (statusText) statusText.innerText = 'Playing clear English audio...';
      if (heroPlayIcon) heroPlayIcon.innerText = 'pause';
    };

    utterance.onend = function() {
      if (statusText) statusText.innerText = 'Click to hear clear audio';
      if (heroPlayIcon) heroPlayIcon.innerText = 'volume_up';
    };
...
window.checkAnswer = function(btn, isCorrect) {
  ...
  buttons.forEach(b => {
    b.classList.remove('border-emerald-500', 'bg-emerald-500/20', 'border-rose-500', 'bg-rose-500/20');
  ...
  if (isCorrect) {
    ...
    feedback.classList.remove('hidden', 'bg-rose-500/20', 'text-rose-300', 'border-rose-500/30');
    feedback.classList.add('bg-emerald-500/20', 'text-emerald-300', 'border', 'border-emerald-500/30');
    feedback.innerHTML = '🎉 <b>बिलकुल सही! (Correct!)</b> Present Continuous tense में "Are you coming..." का सही उपयोग किया गया है। +50 XP Awarded!';

    const xpElem = document.getElementById('headerXP');
    if (xpElem) xpElem.innerText = '500 XP';
  } else {
    ...
    feedback.classList.add('bg-rose-500/20', 'text-rose-300', 'border', 'border-rose-500/30');
    feedback.innerHTML = '❌ <b>गलत उत्तर!</b> कल के लिए चल रही क्रिया में "Are you coming" सही रूप है। Option B चुनें!';
  }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        const timer = setInterval(() => {
          ...
          if (count >= target) {
            counter.innerText = target >= 1000000 ? "1M+" : target + "+";
            clearInterval(timer);
          } else {
  ...
  // Ctrl+K Shortcut Handler for Search
