$files = Get-ChildItem "D:\English Vidya\website\data\grammar\dictionary_batches" -Filter "v4_*.json"
$corrupted = @()
...
Write-Output "Total v4 Files checked: $($files.Count)"
...
if ($corrupted.Count -gt 0) {
    ...
    Write-Output "All 64 files are 100% valid JSON arrays!"
}
