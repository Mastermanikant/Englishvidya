...
$outputFile = "D:\English Vidya\website\data\grammar\master_dictionary_FINAL_v5.json"
...
foreach ($f in $files) {
    ...
    # Check if file has a legitimate prefix (mp, v2, v3, v4, v5) or is one of the base files
    $isValid = $false
    if ($name -match "^(mp\d+|v2_|v3_|v4_|v5_)\w+\.json$") {
        $isValid = $true
