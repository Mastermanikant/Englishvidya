...
$outputFile = "D:\English Vidya\website\data\grammar\master_dictionary_FINAL_v4.json"
...
foreach ($f in $files) {
    ...
    # Check if file has a legitimate prefix or is one of the base files
    $isValid = $false
    if ($name -match "^(mp\d+|v2_|v3_|v4_)\w+\.json$") {
        $isValid = $true
